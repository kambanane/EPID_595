...
# Fitting a Bayesian spatial linear regression to point referenced data on house sale price - SOLUTION
...
##  /var/folders/9n/gn30q9z56gv9qbdhfqjyfqn80000gn/T//Rtmp7Q5ebL/downloaded_packages
library(fields)
...
baton.data <- read.table("Baton_Rouge_houses.csv",sep=",",header=TRUE)
dim(baton.data)
## [1] 70 11
...
image.plot(lon.grid,lat.grid,z.mat,zlim=zlim,xlab="Longitude",ylab="Latitude",main="Log selling prices ",xlim=c(range(lon)[1]-0.1,range(lon)[2]+0.1),ylim=c(range(lat)[1]-0.01,range(lat)[2]+0.01))
...
price68.df <- data.frame(cbind(x.coord.68,y.coord.68,logprice.68,living.68,age.68,other.68,baths.68))
price68.df[1:3,]
...
emp.variog.price68 <- variogram(logprice.68~living.68+age.68+other.68+baths.68,locations=~x.coord.68+y.coord.68,data=price68.df,cutoff=5000,boundaries=c(5,10,40,70,500,seq(1500,5000,by=500)))
...
exp.variog.price68 <- fit.variogram(emp.variog.price68,vgm(psill=0.02,"Exp",range=100,nugget=0.0),fit.method=2)
exp.variog.price68
##   model      psill    range
...
fitted.variog68 <- variogramLine(vgm(est.psill.68,"Exp",est.range.68,est.nugget.68),dist_vector=emp.variog.price68$dist)

plot(emp.variog.price68$dist,emp.variog.price68$gamma,xlab="Distance in meters",ylab="Semi-variance",type="p",pch=19,col="coral",ylim=c(0,0.03))
...
# observations.
coords.68 <- as.matrix(cbind(x.coord.68,y.coord.68),nrow=68,ncol=2)
...
# the (full) dataset.
max.dist <- max(rdist(matrix(cbind(x.coord,y.coord),nrow=70,ncol=2)))
max.dist
## [1] 28103.33
...
price68.bayes <- spLM(logprice.68~living.68+age.68+other.68+baths.68,
                      coords=coords.68,starting=list("phi"=phi.ini,"sigma.sq"=sigma2.ini, "tau.sq"=tau2.ini),
                      tuning=list("phi"=0.1, "sigma.sq"=0.01, "tau.sq"=0.01),
                      priors=list("phi.Unif"=int.prior.phi, "sigma.sq.IG"=c(2,scale.sigma2),
                                  "tau.sq.IG"=c(2,scale.tau2),"beta.Flat"), cov.model="exponential",
                      n.samples=50000, verbose=TRUE, n.report=500)
...
price68.bayes.other.pars <- spRecover(price68.bayes, start=burn.in, verbose=FALSE)

names(price68.bayes.other.pars)
##  [1] "p.theta.samples"         "acceptance"             
##  [3] "Y"                       "X"                      
##  [5] "coords"                  "is.pp"                  
##  [7] "cov.model"               "nugget"                 
##  [9] "beta.prior"              "beta.Norm"              
## [11] "x.names"                 "run.time"               
## [13] "p.beta.recover.samples"  "p.theta.recover.samples"
## [15] "p.w.recover.samples"
# Trace plots of the regression coefficients 
...
round(summary(price68.bayes.other.pars$p.theta.samples)$quantiles[,c(1,3,5)],4)
##            2.5%    50%  97.5%
## sigma.sq 0.0055 0.0268 0.0736
## tau.sq   0.0022 0.0113 0.0330
## phi      0.0001 0.0455 0.1894
# And estimates and 95% credible intervals for the regression coefficients.
round(summary(price68.bayes.other.pars$p.beta.recover.samples)$quantiles[,c(1,3,5)],4)
##                2.5%     50%   97.5%
## (Intercept)  9.8865 10.2013 10.5286
## living.68    0.0004  0.0006  0.0007
## age.68      -0.0137 -0.0081 -0.0026
## other.68    -0.0001  0.0001  0.0003
## baths.68    -0.1872 -0.0157  0.1227
# These are the estiamtes (or median) and 95% credible interval for the spatial random effects. 
w.summary <- apply(t(price68.bayes.other.pars$p.w.recover.samples),2,quantile,c(0.025,0.5,0.975))
w.summary
...
image.plot(surf.w$x,surf.w$y,surf.w$z,col=heat.colors(100)[100:1],
           zlim=range(surf.w$z,na.rm=T),xlab="Easting in km",ylab="Northing in km",
           main="Interpolated posterior median \n of spatial random effects")
contour(surf.w$x,surf.w$y,surf.w$z,nlevels=10,add=T,col="black")
...
predcov <- matrix(cbind(rep(1,2),living[69:70],age[69:70],other[69:70],baths[69:70]),nrow=2,ncol=5)
...
pred.last2 <- spPredict(price68.bayes, pred.coords=pred.coords, pred.covars=predcov, start=burn.in, thin=1)
...
post.pred.mean.last2 <- apply(pred.last2$p.y.predictive.samples,1,mean)
...
price68.new.df <- data.frame(cbind(x.coord.68,y.coord.68,logprice.68,age.68))
price68.new.df[1:3,]
##   x.coord.68 y.coord.68 logprice.68 age.68
## 1   690396.2    3363182    10.87616     11
## 2   684576.0    3374180    10.54534      2
## 3   681534.1    3376912    10.59663      0
# The empirical semi-variogram using default choices for bins and number of bins.
emp.variog.price68.new <- variogram(logprice.68~age.68,locations=~x.coord.68+y.coord.68,data=price68.new.df)
plot(emp.variog.price68.new)

# Again the empirical semi-variogram using does not look good. Using a cutoff of 6000, we have
emp.variog.price68.new <- variogram(logprice.68~age.68,locations=~x.coord.68+y.coord.68,data=price68.new.df,cutoff=5000,boundaries=c(5,10,40,70,500,seq(1500,5000,by=500)))
plot(emp.variog.price68.new,ylim=c(0,0.15))

# Fitting an exponential semi-variogram to the empirical semi-variogram built using the default choices 
# for bins and limit on the distance
exp.variog.price68.new <- fit.variogram(emp.variog.price68.new,vgm(psill=0.10,"Exp",range=300,nugget=0.01),fit.method=2)
exp.variog.price68.new
##   model      psill    range
## 1   Nug 0.00000000   0.0000
## 2   Exp 0.09804025 317.3042
est.psill.68.new <- exp.variog.price68.new$psill[2]
est.psill.68.new
## [1] 0.09804025
est.range.68.new <- exp.variog.price68.new$range[2]
est.range.68.new
## [1] 317.3042
est.nugget.68.new <- exp.variog.price68.new$psill[1]
est.nugget.68.new
## [1] 0
# Plotting the empirical and the fitted semi-variogram
fitted.variog68.new <- variogramLine(vgm(est.psill.68.new,"Exp",est.range.68.new,est.nugget.68.new),dist_vector=emp.variog.price68.new$dist)

plot(emp.variog.price68.new$dist,emp.variog.price68.new$gamma,xlab="Distance in meters",ylab="Semi-variance",type="p",pch=19,col="coral",ylim=c(0,0.15))
lines(emp.variog.price68.new$dist,fitted.variog68.new$gamma,col="dodgerblue")

coords.68 <- as.matrix(cbind(x.coord.68,y.coord.68),nrow=68,ncol=2)
##### Defining the prior distributions of the parameters.
shape.sigma2 <- 2
scale.sigma2 <- est.psill.68.new
...
# positive, we add a small number of it.
shape.tau2 <- 2
scale.tau2 <- est.nugget.68.new+0.01
...
# For this, we use the WLS estimates of spatial covariance function parameters, remembering that the 
# nugget effect was estimated to be equal to 0 via WLS and that is not an allowed initial value.
sigma2.ini <- est.psill.68.new
tau2.ini <- est.nugget.68.new+0.01
phi.ini <- 1/est.range.68.new


## Now fitting the Bayesian hierarchical model - using the tuning values as before
price68.bayes.new <- spLM(logprice.68~age.68,
                          coords=coords.68,starting=list("phi"=phi.ini,"sigma.sq"=sigma2.ini, "tau.sq"=tau2.ini),
                          tuning=list("phi"=0.1, "sigma.sq"=0.01, "tau.sq"=0.01),
                          priors=list("phi.Unif"=int.prior.phi, "sigma.sq.IG"=c(2,scale.sigma2),
                                      "tau.sq.IG"=c(2,scale.tau2),"beta.Flat"), cov.model="exponential",
...
par(mai=rep(0.4,4))
plot(price68.bayes.new$p.theta.samples[,1:3])

n.samples <- 50000
burn.in <- 0.80*n.samples
price68.bayes.new.other.pars <- spRecover(price68.bayes.new, start=burn.in, verbose=FALSE)

# Trace plots of the regression coefficients 
par(mai=rep(0.4,4))
plot(price68.bayes.new.other.pars$p.beta.recover.samples)

## Now fitting the Bayesian hierarchical model - using the tuning values provided
price68.bayes.new <- spLM(logprice.68~age.68,
                          coords=coords.68,starting=list("phi"=phi.ini,"sigma.sq"=sigma2.ini, "tau.sq"=tau2.ini),
                          tuning=list("phi"=1, "sigma.sq"=0.1, "tau.sq"=0.5),
                          priors=list("phi.Unif"=int.prior.phi, "sigma.sq.IG"=c(2,scale.sigma2),
                                      "tau.sq.IG"=c(2,scale.tau2),"beta.Flat"), cov.model="exponential",
                          n.samples=50000, verbose=TRUE, n.report=500)
...
# The second set of tuning values is better.
# Calculating the estimates
round(summary(price68.bayes.new.other.pars$p.theta.samples)$quantiles[,c(1,3,5)],4)
##            2.5%    50%  97.5%
## sigma.sq 0.0874 0.1556 0.2659
## tau.sq   0.0019 0.0087 0.0632
## phi      0.0002 0.0010 0.1433
# An estimate for the range parameter is obtained by taking the inverse of the values reported for the 
# median of phi. A 95% credible interval of phi is obtained by taking the inverse of 
# of 97.5-th and 2.5-th percentile for phi.
# And estimates and 95% credible intervals for the regression coefficients.
round(summary(price68.bayes.new.other.pars$p.beta.recover.samples)$quantiles[,c(1,3,5)],4)
##                2.5%     50%   97.5%
## (Intercept) 11.0062 11.2547 11.4758
## age.68      -0.0196 -0.0088  0.0020
# 90% credible intervals are obained by taking the 5th and 90th percentile of the samples.
# That is:
round(apply(price68.bayes.new.other.pars$p.theta.samples,2,quantile,c(0.05,0.95)),4)
##     sigma.sq tau.sq    phi
## 5%    0.0999 0.0024 0.0003
## 95%   0.2407 0.0494 0.0914
# Still inverting the estimates for phi to get the estimates and credible interval for phi
round(apply(price68.bayes.new.other.pars$p.beta.recover.samples,2,quantile,c(0.05,0.95)),4)
##     (Intercept)  age.68
## 5%      11.0514 -0.0179
## 95%     11.4401  0.0001
###
...
predcov <- matrix(cbind(rep(1,2),age[69:70]),nrow=2,ncol=2)

# These are the coordinates of the sites where we want to make predictions
pred.coords <- cbind(x.coord[69:70],y.coord[69:70])

# Here we make predictions using all the iterations after burnin.
pred.last2 <- spPredict(price68.bayes.new, pred.coords=pred.coords, pred.covars=predcov, start=burn.in, thin=1)
