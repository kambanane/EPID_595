...
pm25.data <-
  read.table(
    "Avg_PM25_Feb2019_California_and_other_vars.csv",
    sep = ",",
    header = T
  )
...
# Here we construct the empirical semi-variogram for the residuals of the multiple linear regression model
# regressing daily average PM2.5 concentration on the covariates,
...
fitted.variog <-
  variogramLine(vgm(est.psill, "Exp", est.range, est.nugget),
                dist_vector = emp.variog.pm25$dist)
plot(
  emp.variog.pm25$dist,
  emp.variog.pm25$gamma,
  type = "p",
  pch = 19,
